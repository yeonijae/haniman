...
const PatientSearch: React.FC<PatientSearchProps> = ({ addPatientToConsultation, addPatientToTreatment, allPatients, updatePatientInfo, deletePatient }) => {
  ...
  useEffect(() => {
    if (selectedPatient && !allPatients.some(p => p.id === selectedPatient.id)) {
        handleBackToSearch();
    }
            ...
            {searchResults.map((patient) => (
              <li key={patient.id} className="p-3 flex justify-between items-center hover:bg-blue-50 cursor-pointer" onClick={() => handleSelectPatient(patient)}>
                <div>
                  ...
                  <p className="text-sm text-clinic-text-secondary">{patient.details} / {patient.status}</p>
                </div>
